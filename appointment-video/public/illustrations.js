...
//
// Each scene is a little widescreen "film shot" (360 x 202) with
// three storytelling layers:
//
//   1. THE HERO. The same patient — the one in the warm red
//      jumper (person({hero: true})) — appears across the scenes,
//      so the video reads as one person's story, not clip art.
//   2. CHOREOGRAPHY. Pieces of each scene enter in order when the
//      scene starts: setting (in-1), main prop (in-2), people
//      (in-3), then the action accents (in-4). Defined once in
//      styles.css; skipped under prefers-reduced-motion.
//   3. ATMOSPHERE. Soft per-scene light (morning gold, clinic
//      mint, night blue) and gentle ground shadows for depth.
//
// The shared palette (keep to these — it's what makes the scenes
// feel like one storybook):
//   INK    dark warm grey — every outline
//   ACCENT warm coral     — the hero's jumper + each scene's highlight
...
// A vertical light gradient for the frame — each scene gets its own id
// so two scenes on screen at once (during a crossfade) can't collide.
const skyWash = (id, top, bottom) =>
  `<defs><linearGradient id="${id}" x1="0" y1="0" x2="0" y2="1">` +
  `<stop offset="0" stop-color="${top}"/><stop offset="1" stop-color="${bottom}"/>` +
  `</linearGradient></defs>` +
  `<rect x="-2" y="-2" width="364" height="206" fill="url(#${id})" stroke="none"/>`;

// A soft shadow on the ground, anchoring a character or prop.
const shadow = (x, y, rx = 24) =>
  `<ellipse cx="${x}" cy="${y}" rx="${rx}" ry="4.5" fill="${INK}" opacity="0.09" stroke="none"/>`;
...
//   hero      true = this is OUR patient: they wear the warm red jumper,
//             so the same character is recognisable in every scene
//   color/sw  stroke colour and width (dark scenes pass CREAM)
function person(o) {
  const {
    x, feetY, h = 92, lean = 0, look = 0,
    armL = 18, armR = -18, legL = 9, legR = -9,
    face = "smile", cls = "", hero = false, color = INK, sw = 6,
  } = o;
  ...
  // The hero's jumper: the torso drawn thicker, in the accent colour.
  const torsoColor = hero ? ACCENT : color;
  const torsoW = hero ? sw + 2.5 : sw;
  ...
  return (
    ...
    `<path d="M${x} ${hipY.toFixed(1)} L${shoX.toFixed(1)} ${shoY.toFixed(1)}" stroke="${torsoColor}" stroke-width="${torsoW}"/>` +
...
// ============================================================
// The 15 scenes. Layer order everywhere: wash, then
// in-1 setting → in-2 main prop → in-3 people → in-4 action.
// ============================================================
const ILLUSTRATIONS = {
  ...
  doctor_patient: svgWrap(`
    ...
    <g class="in-3">
      ...
      <!-- our patient, in their red jumper, listening and nodding -->
      ${person({ x: 258, feetY: 170, h: 92, look: -1, armL: 50, armR: -14, hero: true, cls: "anim-nod" })}
    </g>
    <g class="in-4">
  ...
  taking_tablet: svgWrap(`
    ${skyWash("g_tablet", "#f8ecd8", "#fbf5ea")}
    <g class="in-1">
    ...
    <g class="in-3">
      ${shadow(150, 174, 26)}
      <!-- our patient, facing the tablet, hand rising to mouth -->
      ${person({ x: 150, feetY: 170, h: 98, look: -1, armL: 24, armR: -120, legL: 7, legR: -7, face: "open", hero: true, cls: "anim-sip" })}
    </g>
    <g class="in-4">
  ...
  blood_test: svgWrap(`
    ...
    <g class="in-2">
      ...
      <!-- the test tube, big and centre-stage -->
  ...
  drinking_water: svgWrap(`
    ...
    <g class="in-3">
      ${shadow(252, 174, 25)}
      <!-- our patient, raising their own cup -->
      ${person({ x: 252, feetY: 170, h: 96, look: -1, armL: 20, armR: -118, face: "open", hero: true, cls: "anim-sip" })}
  ...
  walking: svgWrap(`
    ...
    <g class="in-3">
      ${shadow(208, 172, 28)}
      <!-- our patient, mid-stride -->
      ${person({ x: 208, feetY: 168, h: 100, lean: 4, look: 1, armL: 30, armR: -30, legL: 20, legR: -20, hero: true, cls: "anim-walk" })}
    </g>
    <g class="in-4">
  ...
  phone_call: svgWrap(`
    ...
    <g class="in-3">
      ${shadow(96, 174, 25)}
      <!-- our patient, phone held up to their ear -->
      ${person({ x: 96, feetY: 170, h: 98, look: 1, armL: 91, armR: -18, face: "open", hero: true, cls: "anim-nod" })}
  ...
  question: svgWrap(`
    ...
    <g class="in-3">
      ${shadow(122, 174, 26)}
      <!-- our patient, puzzled, hand to chin -->
      ${person({ x: 122, feetY: 170, h: 100, look: 1, armL: 26, armR: -108, hero: true, cls: "anim-nod" })}
    </g>
    <g class="in-4">
      ...
      <!-- one big gentle question, with a smaller echo -->
  ...
  family: svgWrap(`
    ${skyWash("g_family", "#f7eed7", "#fbf5ea")}
    <g class="in-1">
      ${sun(48, 42, 18)}
    ...
    <g class="in-3">
      ...
      ${shadow(180, 174, 18)}
      ${shadow(240, 174, 24)}
      <!-- our patient, their partner and the little one — hands joined -->
      ${person({ x: 122, feetY: 170, h: 98, look: 1, armL: 48, armR: -16, hero: true, cls: "anim-nod" })}
  ...
  thumbs_up: svgWrap(`
    ${skyWash("g_thumbs", "#f8efdc", "#fbf5ea")}
    <g class="in-1">
      ${sun(52, 44, 16)}
      ${ground()}
    </g>
    ...
    <g class="in-3">
      ${shadow(118, 174, 26)}
      <!-- our patient, celebrating, arms up wide of the head -->
      ${person({ x: 118, feetY: 170, h: 98, look: 1, armL: 112, armR: -112, face: "open", hero: true, cls: "anim-cheer" })}
    </g>
    <g class="in-4">
      <!-- confetti sparks -->
